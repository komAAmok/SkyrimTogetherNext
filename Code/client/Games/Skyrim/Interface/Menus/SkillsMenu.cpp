...
static TiltedPhoques::Initializer s_skillsMenuInit(
    []()
    {
        ...
        if (auto* pProcessMessage = GamePatch::Anchor(52510, "stats menu"))
        {
            ...
            GamePatch::Nop(GamePatch::At(pProcessMessage, {0x84E, 0x929, "1.5.97"}, "stats menu appear"), 6,
                           "stats menu appear");
            // Prevent setting kFreezeFrameBackground flag
            GamePatch::Nop(GamePatch::At(pProcessMessage, {0xA10, 0xAEC, "1.5.97"}, "stats menu background"), 4,
                           "stats menu background");
            // Keep the menu updated
            GamePatch::Nop(GamePatch::At(pProcessMessage, {0x1040, 0xFBC, "1.5.97"}, "stats menu update"), 2,
                           "stats menu update");
        }
        ...
        {
            GamePatch::Nop(GamePatch::At(pControlPatch, {0x46, 0x46, "1.5.97"}, "stats menu controls"), 4,
                           "stats menu controls");
            GamePatch::Nop(GamePatch::At(pControlPatch, {0x4A, 0x4A, "1.5.97"}, "stats menu controls"), 2,
                           "stats menu controls");
        }
    });
