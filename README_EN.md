...
> ## 📦 Recommended mod version: **1.1.1** ⭐ **stable**
>
> 1.5.97 is the version this framework is best validated against, and 1.0.41 fixes the
> deployment and multiplayer crash issues.
> **1.1.1 is the mod version designated as stable**: three audit rounds, and the build with
> the most real-session validation behind it. Newer builds (1.1.2 - 1.1.4) still ship and still
> play, they are simply not the designated stable one - take a newer build for new features,
> stay on 1.1.1 for stability.
> If you are on another game version or an older mod build, please update to these before
> reporting a problem.
...
### ✅ Formerly misreported as a conflict, now compatible: KiLoader / KreatE

**KiLoader** and the plugins that ride on it - **KreatE**, **AELAS** - used to pop
`KiLoader initialization failed` / `Couldn't open logging file`, which reads as this framework being
incompatible with them. **It never was, and it is fixed**: on this build they run alongside each other,
with nothing to uninstall and no either/or choice.

**What actually happened**: the multiplayer overlay (CEF) starts a helper process, `TPProcess.exe`,
which lives in the **game root**. When Windows resolves its imports it looks in the executable's own
directory **before** `System32`, and `libcef.dll` delay-loads `d3d11.dll` by name - which is precisely
the name ENB, ReShade and SpecialK use for the proxy DLL they mount themselves with. So the helper
loaded the **game root's ENB proxy** as if it were the system DLL, ENB pulled in the satellites under
`enbseries\`, and `KiLoaderSatelliteENB` started a second copy of KiLoader with it.

**Why that copy failed**: KiLoader derives its log path from the **running executable's name**, so that
copy became `KiLoaderTPProcess` and tried to open the `Data\KiLoader\KiLoader.log` the game process
holds for the entire session. Two copies of one framework cannot share that file, so the second died
during startup - and the popup made it look like "Skyrim Together conflicts with KiLoader".

**The fix**: `TPProcess.exe` now loads the `System32` copies of `dxgi.dll`, `d3d11.dll`, `d3d12.dll`
and `dcomp.dll` **by full path, before CEF initialises** - the four names that are both delay-loaded by
`libcef.dll` and actually used by graphics frameworks as their proxy. The loader matches an
already-loaded module by base name first, so the later load-by-name hits those instead of the game
root's proxy. `dxgi` must come **before** `d3d11`: `d3d11.dll` statically imports `dxgi.dll`, so
loading `d3d11` first resolves its own dependency against the proxy and fails (measured:
`ERROR_BAD_EXE_FORMAT (193)`).

> Only these names are pinned rather than the whole search path, because this framework's own overlay
> runtime (`libEGL.dll`, `libGLESv2.dll` and friends) is deployed into the game root and found by name
> too.

> On an **older build** you can confirm the chain by moving ENB's proxy DLLs (`d3d11.dll` / `dxgi.dll`)
> out of the game root for one launch: if the popup stops, that was the path.
...
| --- | --- | --- |
| 1.1.5 | 2026-09-27 | **CI compile failure fixed**: both `Build windows` and `Playable Skyrim Together Build` were red on a `DAVSyncTogether` `error C7595` (an `fmt` compile-time check), and the cause was **this repository's own patch** - `Code/plugins/patches/DAVSyncTogether/0001-config-index-enumeration.patch` wrote the placeholder as `"{"}`, putting the escaped quote *inside* the braces, so `fmt` read it as a format spec that `std::string` has no formatter for; restored to `"{}"` with the patch semantics untouched, and `plugin_patches.py` `verify` / `check` now scan added lines and reject any placeholder containing a quote, naming the correct form (negative-tested), because `git apply` can only prove a patch applies, never that it compiles; **the KiLoader / KreatE "incompatible" popup is fixed** - the `KiLoaderTPProcess` in that error names **this framework's own CEF helper** `TPProcess.exe` being hijacked: `libcef.dll` delay-loads `d3d11.dll` / `dxgi.dll` / `d3d12.dll` / `dcomp.dll` by name, the helper sits in the game root, and the loader searches the executable's own directory first, so it loaded the **game root's ENB proxy**, ENB pulled in the satellites under `enbseries\` and a second KiLoader started, the two copies fighting over one log file and dying at startup; the fix preloads those four System32 DLLs by full path before CEF initialises (`dxgi` must come first - `d3d11.dll` statically imports it, and the wrong order fails with `ERROR_BAD_EXE_FORMAT (193)`), all four measured as pinned; **version policy**: the recommended build is **1.1.1**, marked with ⭐ as the stable row in both README tables (the other rows lose their bold), and the `recommended-version` gate now asserts the banner matches the ⭐ row and that exactly one row is marked; this section also carries two items committed earlier but never released: **upstream #901** (a remote actor's Havok controller sitting at `deltaTime = 0`, which makes loose objects slide abnormally when stepped on; the new `hkStepInfo` and the `bhkCharacterController::stepInfo` offset `0x80` were **measured** against a real 1.5.97 executable, with address-library symbols naming both ends, not copied from upstream), and the **`check_docs.py` documentation gate** (coverage, both README version tables, tag-versus-changelog sections, and the recommended version, wired into CI) |
