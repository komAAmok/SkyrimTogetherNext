...
> **数字已过时,请看 [missing_1_5_97_ids.txt](missing_1_5_97_ids.txt)。**
>
> 本文件写于 `Tools/ida/recover_1597.py` 之前,当时剩余 14 个目标
> (4 个函数 + 10 个数据静态变量),覆盖率写作 99.5%(3044/3058)。
> 该脚本之后又恢复了其中若干,当前权威口径一律以
> [missing_1_5_97_ids.txt](missing_1_5_97_ids.txt) 为准——**不要在本文件里
> 复述具体数字**,它已经跟着口径变过好几次(最近一次:生成器漏扫全部 `.h`
> 且把注释掉的调用算作活的,修正后为 3069/3080,99.6%)。
>
> 下面保留的是这些目标的**深入分析线索**(1.6.1170 侧的结构、已排除的
> 候选、下一步该看哪里),这部分仍然有效,也是继续恢复剩余目标的起点;
> 只是"还剩几个"要以 ids.txt 为准。
