...
引用的 **99.6%** 地址,即 3082 个地址中已解析 3069 个),随 mod 安装时会一并
复制到 `Data/SKSE/Plugins/`。全部 3698 条映射都能在官方地址库中找到对应符号。

**务必知悉**:剩余 13 个地址在 1.5.x 上仍未恢复,运行时安全降级(空实现桩 /
...
降级方式分两类,**未映射的 id 是拿来调用还是拿来读,处理不同**:
**函数** id 会拿到"调用即返回 0"的桩,按常规写法调用是安全的;
**数据** id 不能这样——桩是一段可执行内存,把它当数据解引用读到的是那段机器码,
所以这类 id(如物理步长全局、`GarbageCollector` 单例)一律用
`FindAddressById` 显式解析,解析不到就降级。
